...
# for optim in optims:
#     disagreement[optim] = np.zeros((10, 10))
#     for idx in range(10):
#         model1 = MLP(3 * 32 * 32, 512, 3, 10, True).to(device)
#         model1.load_state_dict(
#             torch.load(f"models/cifar/model_{optim}_{idx}.pth")
#         )
#         for jdx in range(10):
#             if idx == jdx:
#                 continue
#             elif idx > jdx:
#                 disagreement[optim][idx, jdx] = disagreement[optim][jdx, idx]
#             elif idx < jdx:
#                 model2 = MLP(3 * 32 * 32, 512, 3, 10, True).to(device)
#                 model2.load_state_dict(
#                     torch.load(f"models/cifar/model_{optim}_{jdx}.pth")
#                 )
#                 disagreement[optim][idx, jdx] = get_disagreement(
#                     model1, model2, test_loader
#                 )

# # save the disagreement dictionary
# torch.save(disagreement, "logs/cifar/disagreement_hyp.pth")

# load the disagreement dictionary
disagreement = torch.load("logs/cifar/disagreement_hyp.pth")

# visualize disagreement matrices
# 4 subplots
...
for i in range(2):
    for j in range(2):
        ...
        axs[i, j].set_title(optims[2 * i + j])
# save the figure
plt.tight_layout()
plt.savefig("disagreement_cifar_hyp.png", dpi=600, bbox_inches="tight")
plt.close()
