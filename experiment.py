    ...
    for sym in ["naive", "scale", "perm"]:
        for data in ["train", "test"]:
            if sym == "naive":
                ...
                # normalize weights
                for i in range(num_models):
                    models[i] = models[i].normalize_weights()
        ...
        for i, width in enumerate(widths):
            ...
            for j in range(int_losses.shape[0]):
                for k in range(int_losses.shape[1]):
                    axes[i // 4, i % 4].plot(
                        int_losses[j, k], "-", color="grey", alpha=0.1
                    )
            ...
            # set y axis limits
            axes[i // 4, i % 4].set_ylim(1e-3, 3.0)
            # set title
...
for sym in ["naive", "scale", "perm"]:
    for data in ["train", "test"]:
        ...
        for i, width in enumerate(widths):
            ...
            axes[i // 4, i % 4].imshow(
                barriers, cmap="viridis", vmin=1e-3, vmax=3.0, norm=LogNorm()
            )
            # title
            axes[i // 4, i % 4].set_title(f"width {width}")
        ...
        im = axes[0, 0].imshow(barriers, cmap="viridis", vmin=1e-3, vmax=3.0)
        fig.colorbar(im, cax=cbar_ax, ticks=[1e-3, 1e-2, 1e-1, 1e0, 1e1, 1e2, 1e3])
...
for sym in ["naive", "scale", "perm"]:
    for data in ["train", "test"]:
        ...
        for i, width in enumerate(widths):
            ...
            # plot heatmap in log scale
            axes[i // 4, i % 4].imshow(
                max_barriers, cmap="viridis", vmin=1e-3, vmax=3.0, norm=LogNorm()
            )
            # title
            axes[i // 4, i % 4].set_title(f"width {width}")
        # common colorbar (log scale)
        fig.subplots_adjust(right=0.8)
        cbar_ax = fig.add_axes([0.85, 0.15, 0.02, 0.7])
        im = axes[0, 0].imshow(max_barriers, cmap="viridis", vmin=1e-3, vmax=3.0)
        fig.colorbar(im, cax=cbar_ax, ticks=[1e-3, 1e-2, 1e-1, 1e0, 1e1, 1e2, 1e3])
